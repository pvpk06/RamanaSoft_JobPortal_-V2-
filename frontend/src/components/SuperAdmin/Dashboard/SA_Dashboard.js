...
import SAAnalysis from '../Quiz/Analyze';
...
const SADash = () => {
  ...
  const { '*': currentPath } = useParams(); // Gets the wildcard part of the path
  const jobId = currentPath.startsWith('job_desc/') ? currentPath.split('/')[1] : null;
  const menuItems = [
    ...
    },
    {
      ...
      submenu: [
        { id: 'allStudents', name: 'All Interns', icon: 'fas fa-list' },
  ...
  const renderContent = () => {
    if (currentPath.startsWith('job_desc/')) {
      ...
      return <SAJobDesc jobId={jobId} />;
    }
    if (currentPath.startsWith('student/')) {
      const candidateID = currentPath.split('/')[1];
      return <SAStudentDetails candidateID={candidateID} />;
    }
    ...
    const statusMapping = {
      ...
      "not-interested-students": "not-interested",
