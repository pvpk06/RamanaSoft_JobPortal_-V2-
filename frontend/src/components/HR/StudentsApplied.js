...
const JobApplicationsTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiService.get(`/hr-job-applications?companyName=${selectedCompany}&hrId=${HrId}`); // Adjust the URL as needed
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await apiService.get('/view-companies');
